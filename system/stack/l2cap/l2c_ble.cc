...
void L2CA_Consolidate(const RawAddress& identity_addr, const RawAddress& rpa) {
  ...
  log::info("consolidating l2c_lcb record {} -> {}", ADDRESS_TO_LOGGABLE_CSTR(rpa), ADDRESS_TO_LOGGABLE_CSTR(identity_addr));
...
void l2cble_process_data_length_change_event(uint16_t handle,
  ...
  if (is_legal_tx_data_len(tx_data_len)) {
    if (p_lcb->tx_data_len != tx_data_len) {
      log::debug("Received data length change event for device:{} tx_data_len:{} => {}", ADDRESS_TO_LOGGABLE_CSTR(p_lcb->remote_bd_addr), p_lcb->tx_data_len, tx_data_len);
      ...
      log::debug("Received duplicated data length change event for device:{} tx_data_len:{}", ADDRESS_TO_LOGGABLE_CSTR(p_lcb->remote_bd_addr), tx_data_len);
    }
  } else {
    log::warn("Received illegal data length change event for device:{} tx_data_len:{}", ADDRESS_TO_LOGGABLE_CSTR(p_lcb->remote_bd_addr), tx_data_len);
  }
...
void l2cble_sec_comp(const RawAddress* bda, tBT_TRANSPORT transport,
  ...
  if (!p_lcb) {
    log::warn("security complete for unknown device. bda={}", ADDRESS_TO_LOGGABLE_CSTR(*bda));
    return;
  }
